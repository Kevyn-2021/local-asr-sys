...
- **v2.49 精度动态分配（默认 auto）**：`ASR_CONFIG["torch_dtype"]` 默认 **`auto`**——pipeline 按音频时长决策：时长 ≥ `torch_dtype_big_s`（默认 **1800s = 30 分钟**）→ `bfloat16`（内存约减半、3.14× 实时）；小于阈值 → `float32`（1.11× 实时）。决策发生在 `pipeline.process_file` 第 (7) 步（`duration_s` 已知），以 `torch_dtype` 参数传入 `QwenAsr`；环境变量可覆盖：`ASR_TORCH_DTYPE=float32|bfloat16|auto`、`ASR_TORCH_DTYPE_BIG_S=<秒>`。背景（v2.48 实测）：58.6 分钟大文件 FP32 下 ASR 峰值 ~15GB，两次 OOM；bf16 正常。阈值依据：FP32 成功处理过的最大文件 23.6 分钟，30 分钟留足余量，可按机器内存调整。
...
### 4.6 模型下载与代理配置
- **网络可达性（v2.51 起敏感细节迁移至 [SEC](./SEC_local_asr_notes.md)，不入库）**：能否直连海外（含 huggingface.co/Google）取决于 ThinkPad 所处网络环境（办公室可直连、家网需本地代理，规则见 SEC）；**hf-mirror.com 兜底可用**——`HF_ENDPOINT=https://hf-mirror.com bash step2_download_models.sh`（覆盖 v2.31/v2.35 "hf-mirror 不可用"的旧结论，网络环境变化所致）。
...
- **本地代理（open_proxy）**：开启/关闭/状态命令、内核位置、端口与环境变量口径详见 [SEC](./SEC_local_asr_notes.md)（不入库）。**脚本化/无 sudo 场景**：优先 `HF_ENDPOINT=https://hf-mirror.com` 或开发机中转；代理已开启的 shell 中设置 `HTTPS_PROXY` 指向本地代理端口即可让下载走代理。
...
- **部署地址可配置（v2.23）**：ThinkPad 常随网络环境切换地址，`deploy_webui.sh` 的 `REMOTE_HOST` 支持 `ASR_REMOTE_HOST=kevin@<当前IP>` 环境变量覆盖（默认地址见 [SEC](./SEC_local_asr_notes.md)）。v2.23 部署验证：平铺重构后开发机与运行节点 **18 个运行时文件 md5 全量一致**，`run.sh` 的 `PROJ_ROOT` 修复在运行节点生效（`/home/kevin/asr_sys_local/asr-local`）。
