...
- 面板 2「声纹簇·标注学习」（v2.68 重构）：**筛选说话人 → 查看发言 → 直接标注**——顶部**说话人下拉**（全部声纹簇 + 声纹库命名的无簇说话人，带标注态标识：已标注 /（未标注）/ 🚫 不标注），选中后展示该说话人**最近的发言**（绝对时间 + 源文件 + 文本，限 100 条），底部为**标注操作区**：未标注 →「标注为某人」+「设为不标注」；已标注 →「改标为他人」+「改回未知」（两步确认）；不标注中 →「恢复标注」；标注即写回 `speaker_clusters.assigned_name` 并自动建人物档案（FR-003-CLUSTER）。**原「处理记录」页的说话人筛选功能已并入此处**，不再需要跨页先看发言再去标注
