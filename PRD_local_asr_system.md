...
- **增量学习（v2.75 限定）**: **只有已标注的簇**（`assigned_name` 非空——含自动认出与手工标注）在匹配命中时才做"增量平均"更新簇向量（持续学习），`sample_count` 累积；**纯 unknown / 已取消标注 / 设为不标注（skip_label）的簇只沿用编号、不更新向量**——避免低质量音源（如低码率压缩导致分离/嵌入不准）把多个说话人平均进同一簇造成污染；等高质量音频分离准确、用户手工标注后，该簇才重新进入学习
