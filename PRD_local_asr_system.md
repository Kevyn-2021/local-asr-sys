...
- **优先级**: P0
- **描述**: 将语音转录为文字
- **精度动态分配（v2.58，阈值 v2.65 微调）**: ASR 加载精度默认按**决策时可用内存 + VAD 语音时长**自动选择——可用内存 ≥ 12GB（v2.65 由 13.5GB 下调，16GB 机器实测决策时可用内存 12.7-13.3GB）且语音 ≤30 分钟 → **FP32**（快，1.11× 实时）；否则 → **bf16**（稳，内存约减半）；可用环境变量覆盖（`ASR_TORCH_DTYPE` / `ASR_FP32_MIN_AVAIL_MB` / `ASR_FP32_MAX_SPEECH_S`）
