...
- 面板 2「浏览归档文件」（全宽）：文本备份/归档音频二选一，按月份折叠分组（`<details>`/`<summary>` 原生 HTML）；**归档音频模式下用下拉框选择文件，回听整段音频（浏览器拖动进度，不做段偏移切分），下方滚动显示该音频的完整字幕（说话人经显示层映射 + ASR 文字 + 绝对时间，v2.85 新增）**；文本备份模式下用下拉框选择文件，在限高滚动预览框中查看全文
