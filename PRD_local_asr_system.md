...
- **标注学习行为（v2.75 限定学习对象 / v2.76 改标即重置 / v2.77 场景化落位）**:
  ...
  - **学习开关**：已标注簇（`assigned_name` 非空——含自动认出与手工标注）命中 → "增量平均"更新簇向量（`sample_count` 累积）+ 返回姓名；**纯 unknown / 已取消标注 / 设为不标注（skip_label）的簇命中 → 只返回编号、不学习**——避免低质量音源（如低码率压缩导致分离/嵌入不准）把多个说话人平均进同一簇造成污染
  - **场景表**（与用户视角一一对应）：
    | # | 处理阶段 | 之后你的操作 | 系统行为 |
    |---|----------|--------------|----------|
    | ① | 未命中 → 新建 `unknown_XXXX` | 标注为某人 | 标注＝挂名（向量不变）；之后命中该簇：返回姓名 + 学习 |
    | ② | 未命中 → 新建 `unknown_XXXX` | 设为不标注 | 之后命中：返回编号、不学习 |
    | ③ | 命中已标注簇 | 不修改 | 命中即学习 + 返回姓名，一直如此 |
