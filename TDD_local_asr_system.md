...
| v2.60 | 2026-08-06 | **WebUI 访问控制（§4.8 / PRD FR-008、FR-008-T）**: ① **问题**——8501 原 `ufw allow 8501/tcp`（Anywhere），局域网任何人拿到 `http://<ThinkPad当前IP>:8501` 即可进入管理界面；② **修复**——ThinkPad ufw 删除全局放行，改为仅放行 **Tailscale 网段（100.64.0.0/10）** 与**白名单设备 IP**（当前：开发机 MacBook 10.44.21.97，办公室 DHCP；新设备由用户提供 IP 或接入 Tailscale）；`Default: deny (incoming)` 兜底；③ **为什么不用 MAC 过滤**——MAC 是二层地址、到主机即被剥掉且可伪造；手机普遍启用随机 MAC（ThinkPad 邻居表实测同一随机 MAC 出现在多个 IP），服务器端无法可靠按 MAC 限流；实际可行方案 = 路由器 MAC→固定 IP 绑定 + ufw IP 白名单，或 Tailscale 身份（推荐，跨网络通用）；④ **install_services.sh 同步**——安装脚本默认只放行 Tailscale 网段 + 可选白名单 IP（第 2 个参数空格分隔传入）；⑤ 验证：白名单内 MacBook `curl -I http://10.44.21.23:8501/` → 200；`ufw status verbose` 确认 8501 无 Anywhere 规则；SSH(22)/远程桌面（3389/RustDesk 21115-21119）保持原策略未动 |
