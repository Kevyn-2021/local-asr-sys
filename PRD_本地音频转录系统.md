...
**场景 D：法律/取证场景**
> 需要精确到秒的时间戳作为证据链。系统必须保证时间戳的**不可篡改性**和**可追溯性**。
...
| 信息项 | 查询方式 |
|--------|---------|
| 全部处理记录（带分页） | `SELECT ... ORDER BY absolute_start_time DESC LIMIT 50 OFFSET ?` |
...
页面从上到下的信息顺序：**状态 → 处理进度（仅处理中时显示） → 收件箱·手动处理 → 处理成果 → 音频处理流程 → 系统负担 → 数据位置**，每个区块一个面板。「最近处理」已移至页 2「处理记录」顶部。
