...
- **无发言样本的簇（v2.75 记录，v2.78 默认隐藏）**：声纹簇在**声纹匹配阶段即创建**（早于 ASR 转录与入库）；若该文件的转录行未产生（ASR 无有效文本、文件处理失败/中断、批次重跑等），簇会保留但没有关联 transcript 片段。此类簇**无发言可展示**（如 unknown_0046，v2.85 起不提供片段试听），**v2.78 起在说话人下拉中默认隐藏**（可勾选「显示无发言样本的说话人」查看，供清理/改名等操作）。属正常机制：身份追踪与转录成功解耦，不影响匹配；无需处理，可忽略，或后续提供"清理无样本簇"运维操作
...
- 面板 2「声纹簇·标注学习」（v2.68 重构，v2.78 下拉优化，v2.85 轻量化，v2.87 恢复未标注试听）：**筛选说话人 → 查看发言 → 试听确认（仅未标注）→ 直接标注**——顶部**说话人下拉**（全部声纹簇 + 声纹库命名的无簇说话人，带标注态标识：已标注 /（未标注）/ 🚫 不标注；**v2.78 起已标注按姓名合并为一行、无发言样本的默认隐藏（可勾选显示）、未标注排最前**），选中后：**未标注簇**展示其**发言列表**（绝对时间 + 源文件 + 文本，倒序限 100 条）+ **「🎧 试听发言」**（v2.87 恢复：`SoundFile` seek 只读片段帧，轻量；按「时间 + 文字预览」选一条试听确认）；**已标注/不标注不再加载任何音频片段**（提示去「文件归档」回听整段）；底部为**标注操作区**：未标注 →「标注并回填」+「设为不标注」；已标注 →「改标并回填」+「改回未知」（两步确认，**同一姓名对应多个簇时作用于全部同名簇**）；不标注中 →「恢复标注」；标注即写回 `speaker_clusters.assigned_name` 并自动建人物档案（FR-003-CLUSTER）。**页底「声纹匹配 · 学习看板」（v2.79 起，v2.80 按人展示）**：顶部阈值行（认名 ≥0.65 ｜ 疑似 0.50–0.65 ｜ 学习 ≥0.75），主表**按已标注人员**逐行展示其片段得分分档（高置信≥0.75 / 认名未学习 0.65–0.75 / 疑似 0.50–0.65 / 未识别 <0.50 / 无得分 / 合计 / 待重置簇），附（全部）合计行；底部小字概况（已标注人数/未标注/不标注/无发言样本/待重置），标注流程不显示置信度
...
| v2.85 | 2026-08-07 | **WebUI 轻量化——移除片段试听，标注只服务未标注簇（§8.2 页3/页4 / FR-003-CLUSTER）**: ① 「声纹簇·标注学习」移除「🎧 试听发言」片段回放（细切分 + 100 条回听随数据增长难以选择、整文件切片加载重——「无法加载音频片段」根因），发言列表**仅对未标注簇展示**，已标注/不标注不再加载任何音频片段；② 文件归档「浏览归档文件」归档音频支持**整段回听（无段偏移）** + 滚动字幕（说话人 + ASR 文字 + 绝对时间），搜索结果页移除片段回放；工程细节见 [TDD v2.85](./TDD_local_asr_system.md#6-变更日志) |
