...
- **Web 标注界面（v2.68 重构，v2.78 下拉优化，v2.85 轻量化）**: 数据库页「声纹簇·标注学习」面板，按 **筛选说话人 → 查看发言（仅未标注簇）→ 直接标注** 的单流程组织（说话人下拉带标注态标识）。**v2.85 轻量化**：移除「🎧 试听发言」片段回放（细切分 + 100 条回听随数据增长难以选择、整文件切片加载重），发言列表**仅对未标注簇展示**（文字辅助标注）；已标注/不标注不再加载任何音频片段，如需听音频到「文件归档」回听整段（含滚动字幕）。标注操作区含「标注并回填」/「改标并回填」/「改回未知」（两步确认）/「设为或恢复不标注」。**v2.78 下拉优化**：已标注人员按姓名**合并为一行**（一人对应多簇不再重复出现），无发言样本的说话人**默认隐藏**（可勾选显示），未标注排最前；合并后的改标/改回作用于该姓名下的**全部同名簇**（与按姓名全局回填语义一致）。布局与交互见 §8.2 页 3
...
- 面板 2「声纹簇·标注学习」（v2.68 重构，v2.78 下拉优化，v2.85 轻量化）：**筛选说话人 → 查看发言（仅未标注）→ 直接标注**——顶部**说话人下拉**（全部声纹簇 + 声纹库命名的无簇说话人，带标注态标识：已标注 /（未标注）/ 🚫 不标注；**v2.78 起已标注按姓名合并为一行、无发言样本的默认隐藏（可勾选显示）、未标注排最前**），选中后：**未标注簇**展示其**发言列表**（绝对时间 + 源文件 + 文本，倒序限 100 条，文字辅助标注）；**已标注/不标注不再加载任何音频片段**（提示去「文件归档」回听整段）；**v2.85 起移除「🎧 试听发言」片段回放**（细切分 + 100 条回听随数据增长难以选择、整文件切片加载重）；底部为**标注操作区**：未标注 →「标注并回填」+「设为不标注」；已标注 →「改标并回填」+「改回未知」（两步确认，**同一姓名对应多个簇时作用于全部同名簇**）；不标注中 →「恢复标注」；标注即写回 `speaker_clusters.assigned_name` 并自动建人物档案（FR-003-CLUSTER）。**页底「声纹匹配 · 学习看板」（v2.79 起，v2.80 按人展示）**：顶部阈值行（认名 ≥0.65 ｜ 疑似 0.50–0.65 ｜ 学习 ≥0.75），主表**按已标注人员**逐行展示其片段得分分档（高置信≥0.75 / 认名未学习 0.65–0.75 / 疑似 0.50–0.65 / 未识别 <0.50 / 无得分 / 合计 / 待重置簇），附（全部）合计行；底部小字概况（已标注人数/未标注/不标注/无发言样本/待重置），标注流程不显示置信度
...
- 面板 2「浏览归档文件」（全宽）：文本备份/归档音频二选一，按月份折叠分组（`<details>`/`<summary>` 原生 HTML；**v2.86 起展开内容限高滚动条，文件多时不撑开页面**）；**归档音频模式下用下拉框选择文件，回听整段音频（浏览器拖动进度，不做段偏移切分），下方滚动显示该音频的完整字幕（说话人经显示层映射 + ASR 文字 + 绝对时间，v2.85 新增；**v2.86 起字幕随播放自动滚动并高亮当前句**，纯客户端 JS、零服务端开销）**；文本备份模式下用下拉框选择文件，在限高滚动预览框中查看全文
