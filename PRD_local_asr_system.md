...
- **页面结构**: 4 个页面，顶部用分段控件（`st.segmented_control`）切换，每个页签是独立矩形区块；v2.38 起页首（标题+时间）与页签**合并为顶部锁定导航条**，同排展示并整条吸顶（详见 §8.2）
  - 页 1 — 概览 · 状态（默认首页）
  - 页 2 — 处理记录
  - 页 3 — 声纹库 · 数据库
  - 页 4 — 搜索 · 文件
...
10. **顶部锁定导航条（v2.38）**：页首（标题 + 北京时间）与页签导航合并为**同一行**（`st.columns` 左品牌右导航），整条 `position: sticky` 吸顶——滚动页面时导航始终可见，任意页可随时切换；吸顶条自带页底色 + 底部细分隔线，留白用 padding 实现（sticky 元素的 margin 区域透明，下层内容会从 margin 处透出）

用户在浏览器地址栏输入 IP 后直接看到看板首页。顶部导航为**原生分段控件**，4 个选项：概览 · 状态 / 处理记录 / 声纹库 · 数据库 / 搜索 · 文件，选中项高亮，每个选项边界清晰。页脚固定显示版本时间戳，用于核对部署是否生效。
**v2.38 起**：页首（标题 + 北京时间）与导航同排组成**顶部锁定导航条**，整条吸顶，滚动时始终可见。
...
```
┌──────────────────────────────────────────────────────────────┐
│  ▪ ASR 本地转录系统  2026-08-04 23:35 北京时间                │  ← 顶部锁定导航条
│       ┌──────────┐┌──────────┐┌──────────────┐┌──────────┐   │    （页首+页签同排，
│       │ 概览·状态 ││ 处理记录  ││ 声纹库·数据库 ││ 搜索·文件 │   │     整条吸顶）
│       └──────────┘└──────────┘└──────────────┘└──────────┘   │
│                                                              │
