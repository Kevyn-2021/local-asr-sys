...
|------|------|
| 产品名称 | 本地音频转录与声纹识别系统 |
| 目标平台 | Linux (Ubuntu 24)，单机运行节点（具体设备型号本机维护，不随仓库发布） |
...
- **处理逻辑**:
  ...
  4. 处理失败时，主文件与兄弟文件**一并移入 `error/` 目录**，并生成带时间戳的 `.error.txt` 日志说明失败原因（`error/` 文件名含产生错误的时间戳 `YYYYMMDD_HHMMSS`，防止新旧批次/同名文件重名冲突）；用户排查修复后可手动移回或通过 WebUI「失败文件重新入队并处理」按钮一键移回收件箱重新处理（v2.84；v2.36：失败文件不再留在收件箱，避免下次扫描把次优格式兄弟文件当主格式处理）
- **示例**:
  ```
  收件箱/2026-08-01/ 下有:
...
- **支持的文件名格式** (按匹配顺序)：
  ...
  - `Note-20260806152345` / `voice_20260806152345` / `20260806152345-录音`（全紧凑式：`YYYYMMDDHHMMSS` 14 位无分隔，时间前后可带任意前缀/后缀；v2.74 新增）
  - **实现口径（v2.50 / v2.74 扩展）**：正则按上面顺序尝试（长格式 → 紧凑式 → ISO → 全紧凑式），`re.search` 不锚定——时间戳在文件名任何位置、带任意前后缀均可提取；全紧凑式前后加数字边界（`(?<!\d)`/`(?!\d)`），避免从更长的纯数字串中截取；详细正则见 [TDD §3.5](./TDD_local_asr_system.md#35-时间戳处理)
...
- **失败归档**: 处理失败时，原始音频文件**移入 `/home/kevin/asr_sys_local/audio_inbox/error/` 目录**（重名时附加产生错误的时间戳 `YYYYMMDD_HHMMSS`），同 stem 兄弟文件一并移入（避免下次扫描把次优格式当主格式处理）；同时在 `error/` 下生成带时间戳的 `.error.txt` 日志（`{源文件名}_{YYYYMMDD_HHMMSS}.error.txt`）记录失败原因。用户排查修复后可手动移回或通过 WebUI「失败文件重新入队并处理」按钮一键移回重试（v2.84）。每次新一轮处理启动前（「准备处理收件箱」按钮或自动），旧错误文件（日志 + 音频）自动移入 `error/archived/` 子文件夹，`error/` 根目录只保留当前批次错误
...
- **声纹簇表** (`speaker_clusters`，见 7.1)：
  - `cluster_id` 主键；`label`（`unknown_0001` 式四位编号，全局递增、**不复用**）；`embedding`（聚合声纹向量 float32）；`assigned_name`（用户标注的姓名，NULL = 纯 unknown）；`sample_count`（累积样本数）；`reset_on_next_match`（v2.76：改标后待重置标记，下次命中用新样本替换向量）
...
- **标注学习行为（v2.75 限定学习对象 / v2.76 改标即重置 / v2.77 场景化落位）**:
  - **核心原则**：学习**只发生在"处理阶段的匹配命中"那一刻**，且只对**当时已标注的簇**生效；标注 / 改标 / 取消标注都只是改变"未来的学习开关 + 返回的姓名"，**不重放、不撤销**过去已发生的向量更新（显示层回填只改历史记录的"名字显示"，不改向量）
  - **学习开关（v2.79 认名/学习阈值解耦）**：已标注簇（`assigned_name` 非空——含自动认出与手工标注）命中时**总是返回姓名**；**得分 ≥ `learn_threshold`（默认 0.75）才做"增量平均"**（`sample_count` 累积），**得分在 [0.65, 0.75) 之间只认名、不学习**——防止低置信/低质量样本污染；**纯 unknown / 已取消标注 / 设为不标注（skip_label）的簇命中 → 只返回编号、不学习**；改标重置不受学习阈值限制（用户已人工确认身份，≥0.65 即重新播种）
  - **场景表**（与用户视角一一对应）：
    | # | 处理阶段 | 之后你的操作 | 系统行为 |
    |---|----------|--------------|----------|
    | ① | 未命中 → 新建 `unknown_XXXX` | 标注为某人 | 标注＝挂名（向量不变）；之后命中该簇：返回姓名 + 学习 |
    | ② | 未命中 → 新建 `unknown_XXXX` | 设为不标注 | 之后命中：返回编号、不学习 |
    | ③ | 命中已标注簇 | 不修改 | 命中即返回姓名；得分 ≥0.75 才学习（0.65–0.75 只认名不学） |
    | ④ | 命中已标注簇 | 改标为他人 | 处理时已学的那一次**保留**；改标置"待重置"标记；下次命中：用新样本**替换**向量重新播种（甩掉旧样本），再之后正常学习 |
    | ⑤ | 命中已标注簇 | 改回未知 / 不标注 | 处理时已学的那一次**无法撤回**；之后命中：返回编号、不学习 |
    | ⑥ | 命中 unknown 簇 | 标注为某人 | 标注前不学习（v2.75）；标注后开始学习 |
    | ⑦ | 疑似（0.50 ≤ 得分 < 0.65） | 人工确认 | 返回"疑似:姓名?"、不学习，等确认 |
    | ⑧ | 已标注簇 | 改回后再标注 | 学习开关：关 → 开（若该簇累积过样本，重新标注时触发重置） |
  - **重置规则（v2.76）**：`assign_cluster_name` 在"原已标注且改标为他人"、或"给 `sample_count > 1` 的簇指派姓名"（曾命名过或旧版学习过）时置 `reset_on_next_match=1`；下次处理命中该簇时用新样本**直接替换**向量（`sample_count` 回 1）并清除标记，之后恢复增量学习；**同名重指派不重置**；**纯新建簇（`sample_count=1`）首次标注不重置**，保留该文件作为身份依据
  - **示例**（帮助理解）：
    - 例 A（防污染）：一段 16kHz/32kbps 低码率录音里 3 个说话人被分离并成一个簇 `unknown_0044`（v2.74 实测）。v2.75 起该簇命中时**不会学习**，标注前不会把 3 个人的声音平均进同一个向量。
    - 例 B（改标甩旧账）：`unknown_0008` 曾被标为"Kevin"并累积了 4 个样本，你试听后发现实际是"JingWen"；改标后置"待重置"，下次命中一段干净的 JingWen 音频时向量被**直接替换**，旧的 4 个样本不再拖累匹配。
    - 例 C（首次标注保留依据）：新建的 `unknown_0032`（`sample_count=1`，来自你刚处理并试听过的文件），你标注为"Sofia"——**不重置**，这个文件里的声音就是她后续识别的身份依据。
- **编号规则**: `next_unknown_label = MAX(已用编号) + 1`，四位；已被指派姓名的编号也不复用（标注 0002=某人后，下次从 0003 继续）
- **Web 标注界面（v2.68 重构，v2.78 下拉优化，v2.85 轻量化，v2.87 恢复未标注试听）**: 数据库页「声纹簇·标注学习」面板，按 **筛选说话人 → 查看发言 → 试听确认（仅未标注）→ 直接标注** 的单流程组织（说话人下拉带标注态标识）。**v2.85 轻量化**：已标注/不标注不再加载任何音频片段（提示去「文件归档」回听整段）；发言列表**仅对未标注簇展示**（文字辅助标注）；**v2.87 恢复「🎧 试听发言」但仅限未标注簇**——`SoundFile` 句柄 seek 到段起点只读该段帧（轻量，不再整文件读入），标注前先听声音确认。标注操作区含「标注并回填」/「改标并回填」/「改回未知」（两步确认）/「设为或恢复不标注」。**v2.78 下拉优化**：已标注人员按姓名**合并为一行**（一人对应多簇不再重复出现），无发言样本的说话人**默认隐藏**（可勾选显示），未标注排最前；合并后的改标/改回作用于该姓名下的**全部同名簇**（与按姓名全局回填语义一致）。布局与交互见 §8.2 页 3
- **回填机制**：标注即 `assign_cluster_name` + 自动建人物档案（见 FR-010），并批量更新 `transcripts` 表 `speaker` 字段（大小写不敏感匹配，兼容旧版 `UNKNOWN_XX` 格式）与 `text_backups/` 目录下所有 `.txt`/`.json` 文件中的说话人标签
- **不标注（v2.43 / v2.75 修正）**：陌生人不值得标注时设为**不标注**——保持原编号 `unknown_XXXX`，标注入口隐藏；v2.75 起**照常参与声纹匹配（沿用编号）但不参与向量学习**（与纯 unknown 一致），可随时恢复标注
- **无发言样本的簇（v2.75 记录，v2.78 默认隐藏）**：声纹簇在**声纹匹配阶段即创建**（早于 ASR 转录与入库）；若该文件的转录行未产生（ASR 无有效文本、文件处理失败/中断、批次重跑等），簇会保留但没有关联 transcript 片段。此类簇**无发言可展示、自然无法试听**（如 unknown_0046），**v2.78 起在说话人下拉中默认隐藏**（可勾选「显示无发言样本的说话人」查看，供清理/改名等操作）。属正常机制：身份追踪与转录成功解耦，不影响匹配；无需处理，可忽略，或后续提供"清理无样本簇"运维操作
- **标注即全局生效（显示层映射 + 存量数据回填）**:
  - **显示层映射（主机制，覆盖所有数据库记录）**：数据库 `transcripts.speaker` 始终存原始标签（注册人姓名 / `unknown_XXXX` 编号），不做回填；Web UI 显示层（webui.py `speaker_display_map()` / `disp_speaker()`）把 `unknown_XXXX` 经 `speaker_clusters.assigned_name` 映射为标注姓名展示。用户标注一次，处理记录页（音频处理记录 / 音频处理详情）、声纹标注页与搜索页结果中该编号的**所有历史片段**（无论标注前还是标注后入库）自动显示姓名，不再显示 unknown。这是最轻量、最彻底的方案——无需修改数据库，所有展示层统一生效（见 §8.1.4、§8.2 页 2/页 3/页 4）
...
- **标注校准 (v2.20)**（应对自动标注可能出错，需手工纠正）：
  - **改标为他人**：已标注的簇可改标为另一个姓名——`assign_cluster_name` 覆盖标注，回填匹配串为**原姓名**（此前已回填到 transcripts / 文本备份）；**v2.76 起改标为他人时同时置 `reset_on_next_match=1`**（下次命中重新播种向量，甩掉旧姓名期间累积的样本，见「标注学习行为」）
  - **改回未知**：已标注的簇可改回 `unknown_XXXX`——`unassign_cluster_name` 清空 `assigned_name`，回填到簇的**原编号**（label）。**编号是簇的稳定身份、永不改变**，改回后沿用原编号（如 `unknown_0005`），不产生新编号；该簇重新出现在「声纹簇·标注学习」说话人下拉（标注态回到「（未标注）」），可随时再标注。标注→改回→再标注全程可逆，`unknown_XXXX` 始终代表同一个声音簇。**v2.82 起改回未知同时清除 `reset_on_next_match`**——未标注簇不参与学习/重置，看板「待重置簇」只反映已标注且待重置的簇；再标注时按规则重新置位（行为不受影响）
...
- **准备处理收件箱 (v2.17)**:
  - 点击后执行两个动作，为新一轮处理做好准备：
    ...
    2. **解锁**：若存在残留锁文件（上次异常退出遗留的陈旧锁）则删除，确保新一轮处理能正常获取锁；正在处理中（新鲜锁）时保留锁并提示（v2.84：锁内记录 PID，持有进程已死即视为陈旧，无需等 6 小时）
  - 逻辑顺序：先「准备处理收件箱」，再「开始处理收件箱」，动作更顺
- **处理执行**:
  - 逐个处理所有待处理文件，非交互模式（时间冲突自动接受系统建议）
  - 锁文件防止重复触发（v2.84 起：超过 6 小时或持有 PID 已死即视为陈旧自动失效），状态实时写入 `status.json` 供看板同步
...
- **按钮禁用条件**: 「开始处理」在处理中（锁文件有效/PID 存活）或收件箱无待处理文件时禁用；「准备处理」仅处理中禁用——处理失败状态不禁用，用户可直接重新开始
...
- **标注不覆盖已有档案 (v2.19)**: 「标注并回填」时，若 `persons` 表中已存在该姓名，**保留**已填写的性别/出生年/关系/备注，仅当档案不存在时才自动建档（避免"标注后档案只剩姓名"）
- **改名支持（v2.87）**: 编辑已有人物可修改姓名（唯一、不含空格）——同步 `persons`、`speaker_clusters.assigned_name`、`voiceprints.person_name`、`transcripts.speaker` 原文标签与 `text_backups/` 文本备份；**仅改名不重置学习**——不置 `reset_on_next_match`、不动簇向量（与「改标为他人」本质不同：改名不改身份，改标是重新认定身份）
- **Web 界面**: 声纹库·数据库页「人物档案」面板，展示档案列表 + 新增/编辑表单（姓名/性别/出生年/与我的关系/备注）。**v2.89「编辑已有」为显式加载**：姓名下拉右侧「确定加载」按钮，点击后才把该人物资料回填到表单（Streamlit 控件状态跨 rerun 保留，自动回填会串成上一个选中人的信息）——交互多一步但可靠、可预测
...
| 断点续传 | 处理中断后可从断点恢复 |
| 错误隔离 | 单文件处理失败不影响其他文件 |
| 失败归档 | 处理失败时，原始音频文件与同 stem 兄弟文件**移入 `error/` 目录**，并在 `error/` 下生成带时间戳的 `.error.txt` 日志文件说明原因；文件保留供排查，用户可手动移回或通过 WebUI「失败文件重新入队并处理」按钮一键移回收件箱重新处理（v2.84） |
...
CREATE TABLE speaker_clusters (
    ...
    reset_on_next_match     INTEGER DEFAULT 0        -- v2.76：改标后待重置，下次命中用新样本替换向量
);
...
| 信息项 | 查询方式 |
|--------|---------|
| 全部处理记录（页 2，按音频聚合） | `get_audio_records()`——按 `file_hash` 聚合、按源音频时间从远到近，**无条数上限**（v2.67 起不再按片段罗列）；搜索结果页 `search_records()` 限 100 条、展示前 50 |
...
| 错误日志 | `error/` 根目录存放当前批次错误，`error/archived/` 存放历史归档错误；`error/README.md` 说明目录结构 |
...
5. **布局必须顺应内容**：搜索与文件浏览上下堆叠全宽展示；"就地展开的长内容"一律改为"下拉选择 + 限高滚动预览框"，归档音频**下拉选一段整段回放（不做段偏移切分）+ 滚动字幕**，页面不被撑长
...
页面从上到下的信息顺序：**状态 → 处理进度（仅处理中时显示，含 6 阶段进度条与 3×3 表格） → 收件箱·手动处理 → 处理成果 → 系统负担 → 数据位置 → 音频处理流程**，每个区块一个面板。

#### 页 2 — 处理记录

回答：**"处理了哪些音频？每个音频转写出了什么？"**

- 面板 1「音频处理记录」（v2.67 重构，v2.68 显示归档名）：按**处理过的音频**罗列（不再按片段 ID 拆分，无说话人列）；**编号**按源音频时间从远到近、从 1 开始（显示层稳定编号，最远 = 1）；列：编号 / **归档音频**（显示归档文件名 `archive_name`，不再显示源文件名）/ 时长(min) / 源音频开始时间 / 源音频结束时间 / 开始处理时间 / 处理完成时间（时间格式 `YYYY-MM-DD HH:MM`，不含秒；旧记录无起止时间字段时，完成时间回退 `processed_at`、开始时间显示 "—"）
- 面板 2「音频处理详情」（v2.67 重构，v2.68 显示归档名）：**按音频编号**选择（下拉与标题均显示**归档文件名**），展示该音频的起止时间、**整段音频回放**与**完整转写文本**（每行 `[绝对时间起 - 止] 说话人：文本`，说话人显示标注后的姓名）——不再按片段切分播放，用户按文本时间戳自行拖动定位
...
- 面板 2「声纹簇·标注学习」（v2.68 重构，v2.78 下拉优化，v2.85 轻量化，v2.87 恢复未标注试听）：**筛选说话人 → 查看发言 → 试听确认（仅未标注）→ 直接标注**——顶部**说话人下拉**（全部声纹簇 + 声纹库命名的无簇说话人，带标注态标识：已标注 /（未标注）/ 🚫 不标注；**v2.78 起已标注按姓名合并为一行、无发言样本的默认隐藏（可勾选显示）、未标注排最前**），选中后：**未标注簇**展示其**发言列表**（绝对时间 + 源文件 + 文本，倒序限 100 条）+ **「🎧 试听发言」**（v2.87 恢复：`SoundFile` seek 只读片段帧，轻量；**v2.88 起兼容立体声——二维数组均值混音为单声道**；按「时间 + 文字预览」选一条试听确认）；**已标注/不标注不再加载任何音频片段**（提示去「文件归档」回听整段）；底部为**标注操作区**：未标注 →「标注并回填」+「设为不标注」；已标注 →「改标并回填」+「改回未知」（两步确认，**同一姓名对应多个簇时作用于全部同名簇**）；不标注中 →「恢复标注」；标注即写回 `speaker_clusters.assigned_name` 并自动建人物档案（FR-003-CLUSTER）。**页底「声纹匹配 · 学习看板」（v2.79 起，v2.80 按人展示）**：顶部阈值行（认名 ≥0.65 ｜ 疑似 0.50–0.65 ｜ 学习 ≥0.75），主表**按已标注人员**逐行展示其片段得分分档（高置信≥0.75 / 认名未学习 0.65–0.75 / 疑似 0.50–0.65 / 未识别 <0.50 / 无得分 / 合计 / 待重置簇），附（全部）合计行；底部小字概况（已标注人数/未标注/不标注/无发言样本/待重置），标注流程不显示置信度
- 面板 3「人物档案」：档案列表 + 新增/编辑表单（姓名/性别/出生年/与我的关系/备注/是否已标注声纹），与声纹按姓名关联（FR-010）；「编辑已有」选择姓名后需点「确定加载」才回填资料（v2.89），可改姓名且不重置学习（v2.87）。"是否已标注声纹"列自动判断——若 `speaker_clusters.assigned_name` 中存在该姓名则显示"是"，否则"否"
...
- 面板 1「搜索转录文本」（全宽）：关键词 + 说话人 + 时间范围搜索；结果列表每条显示说话人、绝对时间、来源文件、文本（前 200 字）；结果中的说话人同样经显示层映射显示标注后的姓名（未标注仍显示 `unknown_XXXX` 编号，见 §8.1.4）。**v2.85 起不再提供结果片段回放**（轻量化），听音频统一到下方「浏览归档文件」回听整段
- 面板 2「浏览归档文件」（全宽）：文本备份/归档音频二选一，按月份折叠分组（`<details>`/`<summary>` 原生 HTML；**v2.86 起展开内容限高滚动条，文件多时不撑开页面**）；**归档音频模式下用下拉框选择文件，回听整段音频（浏览器拖动进度，不做段偏移切分）**，下方滚动显示该音频的完整字幕（说话人经显示层映射 + ASR 文字 + 绝对时间，v2.85 新增）；**字幕随播放自动滚动并高亮当前句**——v2.87 起改为 `st.components.v1.html` 组件 iframe 承载字幕与 JS（`st.markdown` 内嵌 `<script>` 经 React innerHTML 注入不执行，v2.86 方案实测无效），纯客户端、零服务端开销；文本备份模式下用下拉框选择文件，在限高滚动预览框中查看全文
...
| **声纹误匹配 (张冠李戴)** | 中 | 中 | 三档阈值（自动 0.65 / 疑似 0.50）+「声纹簇·标注学习」标注校准（未标注可看发言；改标/改回未知）+ 文件归档整段回听确认 + 录入规范（1~3 分钟干净语音） |
...
| v2.68 | 2026-08-06 | **处理记录显示归档名 + 说话人标注流程整合（FR-008 / FR-003-CLUSTER）**: ① 页 2「音频处理记录」「音频处理详情」的音频名称统一显示**归档文件名**（`archive_name`，不再显示源文件名），列头改为「归档音频」；② 「数据库」页「声纹簇·标注学习」重构——替代旧表格（ID/编号/标注为/学习样本数）与三段式 tab，改为**筛选说话人 → 查看其发言 → 直接标注**的单面板流程（原「处理记录」页说话人筛选功能并入），保留标注/改标/改回未知/不标注全部操作；工程细节见 [TDD v2.68](./TDD_local_asr_system.md#6-变更日志) |
| v2.69 | 2026-08-06 | **声纹标注面板支持试听发言（FR-003-CLUSTER）**: ① 「声纹簇·标注学习」发言列表下方新增 **「🎧 试听发言」**——按「时间 + 文字预览」选择后直接回放该条发言的音频片段（复用归档音频 + `segment_start_offset/end_offset` 切分，不引入任何新 ID）；② 标注前先听声音确认说话人，再直接标注；工程细节见 [TDD v2.69](./TDD_local_asr_system.md#6-变更日志) |
| v2.70 | 2026-08-06 | **两处 UI 微调（FR-008 / §8.2）**: ① 「音频处理记录」表格**倒序显示**（编号语义不变：1 = 最远，最大编号在最上方）；② 「数据库」页「声纹怎么来的」面板去掉结尾句前的换行，标注学习循环说明并入同一段落；工程细节见 [TDD v2.70](./TDD_local_asr_system.md#6-变更日志) |
| v2.71 | 2026-08-06 | **三处 UI 微调（FR-008 / §8.2）**: ① 「访问控制」页白名单行——「固定」与「移除」按钮改为同宽居中显示，消除按钮偏向右侧的纵向错位感；② 「音频处理记录」面板说明去掉结尾的「，不再拆分片段」；③ 「声纹簇·标注学习」面板说明去掉「（原「处理记录」页说话人筛选已并入）」括号；工程细节见 [TDD v2.71](./TDD_local_asr_system.md#6-变更日志) |
| v2.72 | 2026-08-06 | **五端一致性 Review（文档工程）**: ① 清理补丁遗留错位——FR-003-CLUSTER「Web 标注界面」改写为 v2.68/69 现行流程（筛选-查看-试听-标注）并收敛与 §8.2 页 3 的重复；显示层映射中的旧面板名（最近处理/片段记录/片段详情）更新为「音频处理记录/音频处理详情」；页 1 布局说明删除「最近处理已移至页 2」残留；页 3 面板 2 按钮名统一为「标注并回填/改标并回填」；风险表「疑似待确认队列」更新为现行「试听确认与校准」；② 版本头部与变更日志两两核对一致（v1.0→v2.72）；工程细节见 [TDD v2.72](./TDD_local_asr_system.md#6-变更日志) |
| v2.73 | 2026-08-06 | **清理 RustDesk 残留目录（ThinkPad 运维）**: v2.61 卸载时移置的残留日志目录 `~/rustdesk_remove_20260806/`（仅 RustDesk 运行日志，无进程/无 dpkg 包/无服务/相关端口已关）确认无用后删除；工程细节见 [TDD v2.73](./TDD_local_asr_system.md#6-变更日志) |
| v2.74 | 2026-08-07 | **文件名时间格式新增全紧凑式 + 中文搜索索引修复（FR-001-TS / FR-008-S）**: ① 文件名时间提取新增 `YYYYMMDDHHMMSS` 14 位无分隔格式（如 `Note-20260806152345`，v2.74 前该文件名无法解析、回退到文件创建时间导致录音开始时间识别错误），并同步 PRD/TDD/settings.py 三处口径；② 修复同文件内重复文本段导致的中文搜索索引（FTS5）同步报 `constraint failed`（转录不受影响但索引缺行）；工程细节见 [TDD v2.74](./TDD_local_asr_system.md#6-变更日志) |
| v2.75 | 2026-08-07 | **声纹学习策略限定 + 无样本簇机制记录（FR-003-CLUSTER）**: ① 增量学习仅限**已标注簇**（自动认出 / 手工标注），纯 unknown / 取消标注 / 不标注簇只沿用编号不更新向量——避免低质量音源污染声纹库；② 记录"无发言样本簇"机制（匹配阶段即建簇、转录未产出行时无音频可试听，属预期）；工程细节见 [TDD v2.75](./TDD_local_asr_system.md#6-变更日志) |
| v2.76 | 2026-08-07 | **改标即重置声纹向量（FR-003-CLUSTER）**: 给簇指派姓名时，若"改标为他人"或"该簇已累积样本（`sample_count>1`）"，置待重置标记，下次处理命中时用新样本**替换**向量重新播种（`sample_count` 回 1）——避免旧姓名期间累积的错误样本持续污染、越用越不准；纯新建簇（`sample_count=1`）首次标注不重置；工程细节见 [TDD v2.76](./TDD_local_asr_system.md#6-变更日志) |
| v2.77 | 2026-08-07 | **标注学习行为场景化落位（FR-003-CLUSTER 文档工程）**: ① 「标注学习行为」从单条概述扩展为 **核心原则 + 学习开关 + 8 场景表 + 重置规则 + 3 个示例** 的结构化小节（对应 未命中/命中 × 标注/不标注/改标/取消 等用户视角，方便理解）；② 声纹簇表字段与 §7.1 库表结构补 `reset_on_next_match`；③ 修正与 v2.75 冲突的旧表述；工程细节见 [TDD v2.77](./TDD_local_asr_system.md#6-变更日志) |
| v2.78 | 2026-08-07 | **声纹标注下拉优化（FR-003-CLUSTER / §8.2 页3）**: ① 已标注人员按姓名**合并为一行**——同一姓名对应多个声纹簇（实测 KevinZH 18 簇）不再重复出现多行；② 无发言样本的说话人**默认隐藏**（可勾选显示），未标注排最前；③ 合并后的改标/改回作用于该姓名**全部同名簇**（与按姓名全局回填语义一致）；④ 实测下拉 46 → 20 行；工程细节见 [TDD v2.78](./TDD_local_asr_system.md#6-变更日志) |
| v2.79 | 2026-08-07 | **声纹认名/学习阈值解耦 + 匹配学习看板（FR-003-CLUSTER / §8.2 页3）**: ① 新增 `learn_threshold`（默认 0.75）——命中已标注簇得分 ≥0.75 才增量学习，0.65–0.75 之间只自动认名不学习，防低置信/低质量样本污染簇向量；改标重置不受限制（已人工确认）；② 数据库页底部新增「声纹匹配 · 学习看板」——只读展示片段得分档位分布、声纹簇概况与已标注人员明细，标注流程不显示置信度；工程细节见 [TDD v2.79](./TDD_local_asr_system.md#6-变更日志) |
| v2.80 | 2026-08-07 | **声纹看板改为按人员展示（FR-003-CLUSTER / §8.2 页3）**: ① 看板主表**按已标注人员**逐行列出片段得分分布（高置信≥0.75 / 认名未学习 0.65–0.75 / 疑似 0.50–0.65 / 未识别 <0.50 / 无得分 / 合计 / 待重置簇），每行 = 该人姓名 + 其全部声纹簇聚合，附（全部）合计行；② 移除"总声纹簇"等笼统计数，改为底部一行小字概况（已标注人数/未标注/不标注/无发言样本/待重置）；③ 阈值行前置（认名 ≥0.65 ｜ 疑似 0.50–0.65 ｜ 学习 ≥0.75）；工程细节见 [TDD v2.80](./TDD_local_asr_system.md#6-变更日志) |
| v2.81 | 2026-08-07 | **五端一致性 Review（文档工程）**: ① TDD §3.3 阈值口径补学习阈值 0.75（v2.79 解耦后原"三档阈值"表述过时）；② 全量运行时文件 md5 复核一致；③ SEC 白名单与交接文档刷新（本机维护，gitignored 不入库）；④ PRD/TDD 版本头与 changelog 核对一致（v1.0→v2.81）；工程细节见 [TDD v2.81](./TDD_local_asr_system.md#6-变更日志) |
| v2.82 | 2026-08-07 | **改回未知清除待重置标记（FR-003-CLUSTER）**: ① `unassign_cluster_name`（改回未知）同时置 `reset_on_next_match=0`——未标注簇不参与学习/重置，看板「待重置簇」只反映已标注且待重置的簇；② 存量休眠标记清理（未标注簇的 `reset_on_next_match` 归零）；③ 再次标注时 `assign_cluster_name` 按 `sample_count>1` 规则重新置位，行为不变；工程细节见 [TDD v2.82](./TDD_local_asr_system.md#6-变更日志) |
| v2.83 | 2026-08-07 | **未加载 .env 时拒绝打开数据库（工程健壮性）**: ① `db.py::connect()` 由 v2.47 的 stderr 告警升级为**直接抛错拒绝**——无显式 `db_path` 且未设置 `ASR_ARCHIVE` 时不再创建默认 `~/audio_archive` 空库（2026-08-07 11:01 临时查询未 source .env 再次制造 0 字节空库残留，已清理）；② 报错信息指引 source .env / run.sh / systemd 或显式传 `db_path`；功能需求无变化，工程细节见 [TDD v2.83](./TDD_local_asr_system.md#6-变更日志) |
| v2.84 | 2026-08-07 | **批处理可恢复性 + 一键重新入队（FR-008-M / FR-001）**: ① webui 重启不再中断批处理——`asr-webui.service` 加 `KillMode=process`，systemd 只终止 webui 主进程，不连坐同 cgroup 的 `process_inbox.py` 子进程（2026-08-07 16:05 部署重启曾把运行中批处理杀成 Diarization 失败）；② 锁文件 PID 感知——锁内记录 PID，持有进程已死即视为陈旧，崩溃/被杀后可立即重新开始，无需等 6 小时；③ 收件箱面板新增「↩️ 失败文件重新入队并处理」按钮——error/ 当前批次失败音频一键移回收件箱并启动；工程细节见 [TDD v2.84](./TDD_local_asr_system.md#6-变更日志) |
| v2.85 | 2026-08-07 | **WebUI 轻量化——移除片段试听，标注只服务未标注簇（§8.2 页3/页4 / FR-003-CLUSTER）**: ① 「声纹簇·标注学习」移除「🎧 试听发言」片段回放（细切分 + 100 条回听随数据增长难以选择、整文件切片加载重——「无法加载音频片段」诱因，**真因后经 v2.88 定位为立体声二维数组**），发言列表**仅对未标注簇展示**，已标注/不标注不再加载任何音频片段；② 文件归档「浏览归档文件」归档音频支持**整段回听（无段偏移）** + 滚动字幕（说话人 + ASR 文字 + 绝对时间），搜索结果页移除片段回放；工程细节见 [TDD v2.85](./TDD_local_asr_system.md#6-变更日志) |
| v2.86 | 2026-08-07 | **文件归档体验增强（§8.2 页4）**: ① 浏览归档文件月份展开列表限高滚动条（文本备份/归档音频文件多时不撑开页面）；② 归档音频字幕**随播放自动滚动 + 当前句高亮**——客户端 JS 按 segment 偏移命中播放进度（零服务端开销）；工程细节见 [TDD v2.86](./TDD_local_asr_system.md#6-变更日志) |
| v2.87 | 2026-08-07 | **未标注试听恢复 + 人物档案改名 + 字幕自动滚动修复（FR-003-CLUSTER / FR-010 / §8.2 页3/页4）**: ① 「声纹簇·标注学习」恢复「🎧 试听发言」但**仅限未标注簇**——`SoundFile` 句柄 seek 只读片段帧（轻量，不再整文件读入）；已标注/不标注仍不加载音频；② 人物档案支持**编辑已有人员（含改姓名与备注）**——`rename_person()` 同步 persons/声纹簇 assigned_name/声纹库/transcripts 原文标签与文本备份，**仅改名不重置学习**（不置 reset_on_next_match、不动向量）；③ 修复归档字幕自动滚动——`st.markdown` 内嵌 `<script>` 不执行，改 `st.components.v1.html` 组件 iframe 承载字幕 + JS 监听播放进度；工程细节见 [TDD v2.87](./TDD_local_asr_system.md#6-变更日志) |
| v2.88 | 2026-08-07 | **试听立体声兼容 + 人物档案表单串信息修复（FR-003-CLUSTER / FR-010 / §8.2 页3）**: ① `render_segment_audio()` 修复「无法加载音频片段」——48kHz 立体声文件 `SoundFile.read` 返回 `(n,2)` 二维数组，`st.audio` 不支持 2D 数组而抛异常；改为二维一律均值混音为单声道（并加空段保护）；② 人物档案「编辑已有」切换人物后各输入框仍显示上一个选中人资料（Streamlit 带 key 控件状态跨 rerun 保留、覆盖 `value` 参数）——人物变化时先清控件状态再回填；工程细节见 [TDD v2.88](./TDD_local_asr_system.md#6-变更日志) |
| v2.89 | 2026-08-07 | **人物档案表单改为显式「确定加载」（FR-010 / §8.2 页3）**: 「编辑已有」选择姓名后需点「确定加载」才把该人物资料写入各输入框（覆盖 Streamlit 跨 rerun 保留的旧控件状态；v2.88 自动清状态方案实测仍串信息）；交互显式、可预测，代价是多点一次；工程细节见 [TDD v2.89](./TDD_local_asr_system.md#6-变更日志) |
| v2.90 | 2026-08-07 | **人物档案「确定加载」按钮样式与对齐（§8.2 页3）**: ① 按钮改 `type="primary"`（主题暖赭 `#b86a48`），醒目防漏点；② 右侧按钮列加等高占位，与左侧「姓名」下拉输入框同行对齐；工程细节见 [TDD v2.90](./TDD_local_asr_system.md#6-变更日志) |
| v2.91 | 2026-08-07 | **PRD/TDD/SEC 三文档一致性 Review（文档工程）**: ① `persons` 表结构补 `created_at/updated_at`（对齐 db.py 实际 schema 与 TDD §1.6 查询）；② §8.1.4「全部处理记录」由旧 `LIMIT 200` 改为现行口径（页 2 按音频聚合无上限，搜索结果限 100/展示 50）；③ `error/README` 后缀 `.txt`→`.md`（对齐运行节点实际文件）；④ FR-008-M 按钮禁用条件修正为代码实际口径（处理失败态不禁用）；⑤ 试听「无法加载音频片段」根因表述统一——v2.85 记为诱因、v2.88 定位真因（立体声二维数组）；⑥ §8.2 页 3/页 4、无发言样本簇、目标平台措辞等细节对齐；工程细节见 [TDD v2.91](./TDD_local_asr_system.md#6-变更日志) |
