...
- **Web 标注界面（v2.68 重构，v2.69 加试听）**: 声纹库·数据库页「声纹簇·标注学习」面板，按 **筛选说话人 → 查看发言 → 试听确认 → 直接标注** 的单流程组织（说话人下拉带标注态标识；发言列表 + 「🎧 试听发言」；标注操作区含「标注并回填」/「改标并回填」/「改回未知」（两步确认）/「设为或恢复不标注」），布局与交互见 §8.2 页 3
...
- **无发言样本的簇（v2.75 记录，预期现象）**：声纹簇在**声纹匹配阶段即创建**（早于 ASR 转录与入库）；若该文件的转录行未产生（ASR 无有效文本、文件处理失败/中断、批次重跑等），簇会保留但没有关联 transcript 片段。此类簇在「声纹簇·标注学习」说话人下拉中显示为 **0 条发言、无音频可试听**（如 unknown_0046）。属正常机制：身份追踪与转录成功解耦，不影响匹配；无需处理，可忽略，或后续提供"清理无样本簇"运维操作
...
- 面板 2「声纹簇·标注学习」（v2.68 重构，v2.69 加试听）：**筛选说话人 → 查看发言 → 试听确认 → 直接标注**——顶部**说话人下拉**（全部声纹簇 + 声纹库命名的无簇说话人，带标注态标识：已标注 /（未标注）/ 🚫 不标注），选中后展示该说话人**最近的发言**（绝对时间 + 源文件 + 文本，限 100 条），发言列表下方新增 **「🎧 试听发言」**——按「时间 + 文字预览」选择后直接回放该条发言的音频片段（复用归档音频 + 段偏移切分，不引入任何新 ID），标注前先听声音确认说话人；底部为**标注操作区**：未标注 →「标注并回填」+「设为不标注」；已标注 →「改标并回填」+「改回未知」（两步确认）；不标注中 →「恢复标注」；标注即写回 `speaker_clusters.assigned_name` 并自动建人物档案（FR-003-CLUSTER）。**原「处理记录」页的说话人筛选功能已并入此处**，不再需要跨页先看发言再去标注
