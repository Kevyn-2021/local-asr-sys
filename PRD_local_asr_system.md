...
- **标注校准 (v2.20)**（应对自动标注可能出错，需手工纠正）：
  ...
  - **改回未知**：已标注的簇可改回 `unknown_XXXX`——`unassign_cluster_name` 清空 `assigned_name`，回填到簇的**原编号**（label）。**编号是簇的稳定身份、永不改变**，改回后沿用原编号（如 `unknown_0005`），不产生新编号；该簇重新出现在「声纹簇·标注学习」说话人下拉（标注态回到「（未标注）」），可随时再标注。标注→改回→再标注全程可逆，`unknown_XXXX` 始终代表同一个声音簇
