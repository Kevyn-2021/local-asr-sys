...
- **增量学习（v2.75 限定 / v2.76 改标即重置）**: **只有已标注的簇**（`assigned_name` 非空——含自动认出与手工标注）在匹配命中时才做"增量平均"更新簇向量（持续学习），`sample_count` 累积；**纯 unknown / 已取消标注 / 设为不标注（skip_label）的簇只沿用编号、不更新向量**——避免低质量音源（如低码率压缩导致分离/嵌入不准）把多个说话人平均进同一簇造成污染；等高质量音频分离准确、用户手工标注后，该簇才重新进入学习。**v2.76 改标即重置**：给簇指派姓名时，若"原已标注且改标为他人"、或"该簇已累积样本（`sample_count > 1`，曾命名过或旧版学习过）"，则置 `reset_on_next_match=1`——下次处理命中该簇时用新样本**直接替换**向量（`sample_count` 回 1）重新播种，避免旧姓名期间累积的错误样本持续污染、越用越不准；**纯新建簇（`sample_count=1`）首次标注不重置**，保留该文件作为身份依据
...
- **标注校准 (v2.20)**（应对自动标注可能出错，需手工纠正）：
  - **改标为他人**：已标注的簇可改标为另一个姓名——`assign_cluster_name` 覆盖标注，回填匹配串为**原姓名**（此前已回填到 transcripts / 文本备份）；**v2.76 起改标为他人时同时置 `reset_on_next_match=1`**（下次命中重新播种向量，甩掉旧姓名期间累积的样本，见"增量学习"）
