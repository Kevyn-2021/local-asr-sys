...
- **优先级**: P0
- **描述**: 将语音转录为文字
- **精度动态分配（v2.58）**: ASR 加载精度默认按**决策时可用内存 + VAD 语音时长**自动选择——可用内存 ≥ 13.5GB 且语音 ≤30 分钟 → **FP32**（快，1.11× 实时）；否则 → **bf16**（稳，内存约减半）；可用环境变量覆盖（`ASR_TORCH_DTYPE` / `ASR_FP32_MIN_AVAIL_MB` / `ASR_FP32_MAX_SPEECH_S`）
- **转录粒度（v2.53/2.54 原则）**: 转录行**不严格按说话人切分**——效率优先（处理时长短）优先于逐说话人粒度；ASR 前会把时间相近的相邻段合并（间隔 ≤1.5s、段长 ≤60s，可配置），合并行归属起始段说话人。声纹身份由**声纹簇**承载、不受转录行粒度影响，声纹标注可通过单人录音等其他途径完成（单人录音 → 一个分离说话人 → 一个簇/ID，见 FR-003-CLUSTER）
- **模型**: Qwen3-ASR-1.7B（v2.32 定稿为显式 **FP32** 加载——CPU oneDNN 优化，实测 1.11× 实时、峰值内存 11.8GB；唯一 ASR 模型，无备用快速模型）
...
- **浏览器访问环境**:
  ...
  - 建议加入浏览器书签，一键打开
  - **访问控制（v2.61）**: 端口 8501 由 ufw 限制为**仅放行 Tailscale 网段（100.64.0.0/10）与白名单设备 IP**，其余来源一律拒绝（ufw `Default: deny (incoming)` 兜底）；白名单可在「访问控制」页直接增删（见 FR-008-A）。新增设备：① 安装/开启 Tailscale（推荐，办公室/家里通用，访问 `http://<ThinkPad-Tailscale-IP>:8501`）；② 或在「访问控制」页添加设备 IP（手机随机 MAC 需关闭该 Wi-Fi 的"私有地址"才能在路由器侧稳定绑定）
- **部署环境（SSH）**:
...
- **页面结构**: 5 个页面，顶部用分段控件（`st.segmented_control`）切换，每个页签是独立矩形区块、**宽度一致（等宽）**；v2.38 起页首与页签**合并为顶部锁定导航条**，同排展示并整条吸顶（详见 §8.2）；v2.61 起北京时间从顶栏移出，改为首页第一个面板
...
- **描述**: 第 5 个页签「访问控制」，含两个面板：
  1. **IP 白名单 · 网页访问**：展示 8501 端口当前放行来源（Tailscale 网段固定放行、设备 IP 可增删），用户可直接新增/移除设备 IP，修改即时生效
  2. **端口说明**：列出 ThinkPad 对外服务端口与用途——SSH 22（开发机远程管理/部署）、Web UI 8501（浏览器访问）；远程桌面端口（3389 / RustDesk）已关闭
- **实现约束**: 白名单操作经 sudoers NOPASSWD 限定的管理脚本（`/usr/local/sbin/asr-webui-fw.sh`）调用 ufw，仅允许操作 8501 端口、仅接受合法 IPv4；Tailscale 网段与回环地址固定放行、不可删除
...
- **要点**:
  ...
  - **端口 8501 防火墙策略（v2.60 起 / v2.61 可在 WebUI 管理）**: ufw 仅放行 Tailscale 网段（100.64.0.0/10）+ 白名单设备 IP；白名单在「访问控制」页直接增删（FR-008-A），Tailscale 网段固定放行
...
| 访问控制 | 数据库文件权限 600；Tailscale ACL 仅放行用户本人设备；WebUI 端口 8501 仅放行 Tailscale 网段 + 白名单设备 IP——白名单在 WebUI「访问控制」页直接增删（v2.61），Tailscale 网段固定放行 |
...
10. **顶部锁定导航条（v2.38 / v2.61）**：页首（品牌标题）与 5 个**等宽**页签合并为**同一行**（`st.columns` 左品牌右导航），整条 `position: sticky` 吸顶——滚动页面时导航始终可见，任意页可随时切换；吸顶条自带页底色 + 底部细分隔线，留白用 padding 实现（sticky 元素的 margin 区域透明，下层内容会从 margin 处透出）；**北京时间 v2.61 起移出顶栏**，改为首页「北京时间」面板（导航条下方第一个面板，仅首页展示）

用户在浏览器地址栏输入 IP 后直接看到看板首页。顶部导航为**原生分段控件**，5 个**等宽**选项：状态概览 / 处理记录 / 数据库 / 文件归档 / 访问控制，选中项高亮，每个选项边界清晰。页脚固定显示版本时间戳，用于核对部署是否生效。
...
│   ┌────────┐┌────────┐┌────────┐┌────────┐┌────────┐        │    （v2.61：品牌+5 个
│   │状态概览 ││处理记录 ││ 数据库 ││文件归档 ││访问控制 │        │     等宽页签，整条吸顶；
│   └────────┘└────────┘└────────┘└────────┘└────────┘        │      时间已移出顶栏）
│  ┌──────────────────────────────────────────────────────┐   │
...
- 面板 1「IP 白名单 · 网页访问」：列出当前 8501 端口放行来源（Tailscale 网段固定、设备 IP 可移除），支持新增/移除设备 IP（即时生效）；修改经 sudoers 限定的管理脚本调用 ufw（见 FR-008-A）
